...
namespace GmatClubTest.DbEditor.Data
{
    ...
    public class Provider : Component
    {
        ...
        private OleDbDataAdapter allTestsAdapter;
        private OleDbDataAdapter allTestContentsAdapter;
        ...
        private OleDbDataAdapter questionSetById;
        ...
        private OleDbDataAdapter questionSetsExByTestId;
            ...
            for (int i = 0; i < test.PassagesToQuestionsEx.Count; ++i)
            {
                for (int j = 0; j < test.QuestionsEx.Count; ++j)
                {
                    if (test.PassagesToQuestionsEx[i].Id == test.QuestionsEx[j].Id)
                    {
                        ...
                        passageQuestions.SelectCommand.Parameters[0].Value =
                        ...
                        if (
                            test.QuestionsEx.FindByIdSetId(test.PassagesToQuestionsEx[i].PassageQuestionId,
                                                           test.QuestionsEx[j].SetId) == null)
                        {
                            test.QuestionsEx.AddQuestionsExRow(sd.PassageQuestions[0].TypeId,
            ...
            for (int i = 0; i < questionSet.PassagesToQuestionsEx.Count; ++i)
            {
                for (int j = 0; j < questionSet.QuestionsEx.Count; ++j)
                {
                    if (questionSet.PassagesToQuestionsEx[i].Id == questionSet.QuestionsEx[j].Id)
                    {
                        ...
                        passageQuestions.Fill(sd.PassageQuestions);
                        if (
                            questionSet.QuestionsEx.FindByIdSetId(
                                questionSet.PassagesToQuestionsEx[i].PassageQuestionId, questionSet.QuestionsEx[j].SetId) ==
                            null)
                        {
                            questionSet.QuestionsEx.AddQuestionsExRow(sd.PassageQuestions[0].TypeId,
                                                                      sd.PassageQuestions[0].SubtypeId,
                                                                      sd.PassageQuestions[0].DifficultyLevelId,
                                                                      sd.PassageQuestions[0].Text, null,
        ...
        public void UpdateQuestionSet(Dataset.QuestionSetsExRow questionSet)
        {
            ...
            c.CommandText = @"UPDATE QuestionSets SET Name = '" + questionSet.Name + "'" +
                            ...
                            ", TimeLimit = " +
                            (questionSet.IsTimeLimitNull() ? ("null") : (questionSet.TimeLimit.ToString())) +
                            ",QuestionTypeId = " +
                            (questionSet.IsQuestionTypeIdNull() ? ("null") : (questionSet.QuestionTypeId.ToString())) +
                            ",QuestionSubtypeId = " +
                            (questionSet.IsQuestionSubtypeIdNull()
                                 ? ("null")
                                 : (questionSet.QuestionSubtypeId.ToString())) +
            ...
            try
            {
                ...
                for (int i = 0; i < fullDs.QuestionSetsEx.Count; ++i)
                {
                    if (fullDs.QuestionSetsEx[i].RowState != DataRowState.Deleted)
                    {
                        c.CommandText =
                            String.Format(
                                "INSERT INTO TestContents(TestId, QuestionSetId, QuestionSetOrder) VALUES({0},{1},{2})",
                                fullDs.Tests[0].Id, fullDs.QuestionSetsEx[i].Id, i);
                        c.ExecuteNonQuery();
                    }
                }
        ...
        public void UpdateSetsToQuestions(Dataset dataset, int setId)
        {
            ...
            for (int i = 0; i < dataset.QuestionsEx.Count; ++i)
            {
                if (dataset.QuestionsEx[i].RowState != DataRowState.Deleted)
                {
                    ...
                    {
                        c.CommandText = "SELECT id FROM Questions WHERE ((TypeId = " + dataset.QuestionsEx[i].TypeId +
                                        ") AND (SubtypeId = " + dataset.QuestionsEx[i].SubtypeId +
                                        ") AND (DifficultyLevelId = " + dataset.QuestionsEx[i].DifficultyLevelId +
                                        ") AND (Text = '" + dataset.QuestionsEx[i].Text + "'))";
                        ...
                        c.CommandText =
                            String.Format(
                                ...
                                setId, iDqr, dataset.QuestionsEx[i].QuestionOrder, dataset.QuestionsEx[i].QuestionZone);
                        c.ExecuteNonQuery();
                    ...
                    else
                    {
                        c.CommandText =
                            String.Format(
                                @"Insert into SetsToQuestions (SetId, QuestionId ,QuestionOrder, QuestionZone) VALUES({0},{1},{2},{3})",
                                setId, dataset.QuestionsEx[i].QuestionId, dataset.QuestionsEx[i].QuestionOrder,
                                dataset.QuestionsEx[i].QuestionZone);
                        c.ExecuteNonQuery();
                        //  numberOfQuestionsToPick++;
                    }
                }
            ...
            c.CommandText = @"UPDATE QuestionSets SET NumberOfQuestionsToPick =" + (numberOfQuestionsToPick) +
                            " Where id = " + setId;
            c.ExecuteNonQuery();
        ...
        public void AddExistingQuestionToSet(Dataset.QuestionsExRow qsr, int order, int setId)
        {
            ...
            c.CommandText =
                String.Format(
                    @"Insert into SetsToQuestions (SetId, QuestionId ,QuestionOrder, QuestionZone) VALUES({0},{1},{2},{3})",
                    setId, qsr.Id, order, qsr.QuestionZone);
            c.ExecuteNonQuery();
        }
        ...
        public void UpdatePassageQuestion(Dataset dataset, Dataset.QuestionsExRow value)
        {
            ...
            for (int i = 0; i < dataset.QuestionsEx.Count; i++)
            {
                ...
                for (int j = 0; j < dataset.PassagesToQuestionsEx.Count; ++j)
                {
                    ...
                    {
                        c.CommandText = "DELETE FROM PassagesToQuestions Where ((PassageQuestionId = " + value.Id +
                                        ")AND( QuestionId =" + questionId + "))";
                        c.ExecuteNonQuery();

                        c.CommandText =
                            String.Format(
                                "Insert into PassagesToQuestions (PassageQuestionId, QuestionId ,QuestionOrder) VALUES({0},{1},{2})",
                                value.Id, questionId, 0);
                        c.ExecuteNonQuery();
                    }
                }
            }
