...
namespace GmatClubTest.DbEditor.Tree
{
	...
	public class Tree : TreeView
	{
		...
		public Tree()
		{
			...
			Entity.StructureInternallyChanged += new GmatClubTest.DbEditor.Tree.Entity.StructureInternallyChangedEventHandler(Entity_StructureInternallyChanged);
		...
		public void SaveTree()
		{
			...
			{
				MessageBox.Show(String.Format("Error while saving the list of connections: {0}", e.Message), ApplicationController.APP_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}
		...
		internal void SetExpanded(Hashtable list, TreeNode node)
		{
			...
			if(list.ContainsKey(eid))
			{
				...
				if (node.IsSelected || ((NodeProperties)list[eid]).IsExpanded) node.Expand(); else node.Collapse();
				if (node.IsSelected) 
				{
					if(node.Parent != null) 
		...
		private void Tree_DragDrop(object sender, DragEventArgs e)
		{
			...
			{
				UnexpectedExceptionDialog.ExceptionBox.Show(ex);
			}
		}
		...
		private void Tree_KeyUp(object sender, KeyEventArgs e)
		{
			if (e.KeyCode == Keys.Delete)
			{
				if (!isEditingNow)
				{
					...
					{
						if(m == this.contextMenuBuilder.deleteCmd) 
						{
