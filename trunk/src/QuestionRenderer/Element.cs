...
namespace GmatClubTest.QuestionRenderer
{
	internal abstract class Element
	{
		...
		public Element(Renderer renderer) {this.renderer = renderer;}
	...
	internal class FormulaElement: Element
	{
		...
		private static Regex regexMtext = new Regex(@"<\s*mtext[^>]*>(.*?)<\s*/mtext\s*>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
		private static Regex regexMtextConcat = new Regex(@"<\s*mtext([^>]*)>(.*?)<\s*/mtext\s*>\s*<\s*mtext\1>(.*?)<\s*/mtext\s*>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
		...
		public FormulaElement(string xmlText, Renderer renderer): base(renderer)
		{
			...
			for(;;)
			{
				...
				stringBuilder.Insert(m.Index, String.Format("<mtext{0}>{1}{2}</mtext>", m.Groups[1], m.Groups[2], m.Groups[3]));
			}
