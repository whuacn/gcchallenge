	...
	internal class TestNavigator: NavigatorSkeleton
	{
		...
		public TestNavigator(TestSet.TestsRow test, Manager manager): base(test, manager)
		{
			...
			foreach (QuestionSetSet.QuestionSetsRow set in sets.QuestionSets)
			{
				...
				++i;
				totalQuestions += set.NumberOfQuestionsInZone1 + set.NumberOfQuestionsInZone2 + set.NumberOfQuestionsInZone3;
		...
		private void GetRandomQuestion(QuestionAnswerSet questionAnswers)
		{
			...
			QuestionAnswerSet.QuestionsRow[] rows = (QuestionAnswerSet.QuestionsRow[])qt.Select("DifficultyLevelId = " + activeDifficultyLevel.ToString());
			if (rows.Length == 0) throw new Exception(String.Format("The content of the test is incorrent. There is not enough questions of difficulty {0} in zone {1} of question set {2}", activeDifficultyLevel, activeZoneIndex + 1, activeSetIndex + 1));
