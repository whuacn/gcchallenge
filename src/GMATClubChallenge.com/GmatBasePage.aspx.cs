...
{
    public partial class GmatBasePage : Page, IStatusHandler
    {
        protected GmatBasePage()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if ("" != fn_ && null != fn_)
                {
                    ...
                    LogManager.GetLogger("access_control").WarnFormat("Access granted for '{0}' to {1}",
                                                                      access_manager_.UserLogin, fn_);
                    on_access_granted();
                }
                try
                {
            ...
            {
                LogManager.GetLogger("access_control").WarnFormat("Access Denied for '{0}' to {1}",
                                                                  access_manager_.UserLogin, fn_);
        ...
        {
            return "<red>Please provide page head in: " + GetType().ToString() + "</red>";
        }
