...
namespace GmatClubTest.DbEditor
{
	public class Editor: UserControl, IDisposable
	{
		...
		protected GmatClubTest.DbEditor.Data.Dataset data;
		...
		public virtual String ObjectName { get{throw new InvalidOperationException("");} }
		public virtual int Id { get{throw new InvalidOperationException("");} }
		...
		{
			if (this.HasChildEditors)
			{
				if (!dontAskToClose)
				{
					DialogResult r = MessageBox.Show(String.Format("This {0} has editing child elements. Do you want to close this {0} and all the children?", this.ObjectName), ApplicationController.APP_CAPTION, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
			...
			if (this.HasChanges)
			{
				DialogResult r = MessageBox.Show("Do you want to save changes to the " + this.ObjectName + "?", ApplicationController.APP_CAPTION, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
				if (r == DialogResult.Yes) this.Save();
			}
