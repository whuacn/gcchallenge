...
namespace GmatClubTest.ImportExport.Data
{
    ...
    public class Provider : Component
    {
        ...
        private OleDbDataAdapter testById;
        ...
        private OleDbDataAdapter questionSubtypes;
        ...
        public DataTable GetQuestionSubtypesTable()
        {
            DataTable table = new DataTable();
            questionSubtypes.Fill(table);
        ...
        public void GetTestById(Dataset test, int testId)
        {
            ...
            for (int i = 0; i < test.PassagesToQuestions.Count; ++i)
            {
                ...
                {
                    questionEditorDataAdapter.SelectCommand.Parameters[0].Value =
                        test.PassagesToQuestions[i].PassageQuestionId;
                    Dataset sd = new Dataset();
                    questionEditorDataAdapter.Fill(sd.Questions);
                    test.Questions.AddQuestionsRow(sd.Questions[0].TypeId, sd.Questions[0].SubtypeId,
                                                   sd.Questions[0].DifficultyLevelId, sd.Questions[0].Text,
                                                   (sd.Questions[0].IsPictureNull() ? (null) : (sd.Questions[0].Picture)));
                }
                else
                {
        ...
        public void SetTest(Dataset test)
        {
            ...
            for (int i = 0; i < test.Questions.Count; ++i)
            {
                if (test.Questions[i].RowState == DataRowState.Deleted)
                {
                    continue;
                }
                ...
                if (i == 0)
                {
                    OleDbCommand getLustQuestionId =
                        new OleDbCommand("Select id from Questions ORDER BY id", connection);
            ...
            for (int i = 0; i < test.PassagesToQuestions.Count; ++i)
            {
                if (test.PassagesToQuestions[i].RowState == DataRowState.Deleted)
                {
                    continue;
                }
            ...
            {
                string comS =
                    @"Insert INTO QuestionSets (Name,Description,NumberOfQuestionsToPick,TimeLimit,QuestionTypeId,QuestionSubtypeId,NumberOfQuestionsInZone1,NumberOfQuestionsInZone2,NumberOfQuestionsInZone3) VALUES (";
                ...
                try
                {
                    comS += ",'" + test.QuestionSets[i].QuestionTypeId == null ? " null " : test.QuestionSets[i].QuestionTypeId + "'";
                }
                catch
                {
                    comS += "," + "null" + "";
                ...
                try
                {
                    comS += ",'" + test.QuestionSets[i].QuestionSubtypeId == null ? " null " : test.QuestionSets[i].QuestionSubtypeId + "'";
                }
                catch
                {
                    comS += "," + "null" + "";
                ...
                {
                    OleDbCommand getLustQuestionId =
                        new OleDbCommand("Select id from QuestionSets ORDER BY id", connection);
                    OleDbDataReader dataReader = getLustQuestionId.ExecuteReader();
            ...
            {
                string comS =
                    @"Insert INTO Tests (Name,IsPractice,Description,QuestionTypeId,QuestionSubtypeId,[GUID],Version) VALUES (";
            ...
            for (int i = 0; i < test.QuestionSets.Count; ++i)
            {
                string comS = @"UPDATE QuestionSets SET NumberOfQuestionsToPick =" +
                              test.QuestionSets[i].NumberOfQuestionsToPick + " Where id = " + (questionSetsStartId + i);
        ...
        private void InitializeComponent()
        {
            ...
            this.questionSubtypes = new System.Data.OleDb.OleDbDataAdapter();
            ...
            // questionSubtypes
            // 
            this.questionSubtypes.DeleteCommand = this.oleDbCommand4;
            this.questionSubtypes.InsertCommand = this.oleDbCommand5;
            this.questionSubtypes.SelectCommand = this.oleDbCommand6;
            this.questionSubtypes.TableMappings.AddRange(new System.Data.Common.DataTableMapping[] {
            ...
            // oleDbCommand4
            // 
            this.oleDbCommand4.CommandText = "DELETE FROM [Questions] WHERE (([Id] = ?))";
            this.oleDbCommand4.Connection = this.connection;
            this.oleDbCommand4.Parameters.AddRange(new System.Data.OleDb.OleDbParameter[] {
            new System.Data.OleDb.OleDbParameter("Original_Id", System.Data.OleDb.OleDbType.Integer, 0, System.Data.ParameterDirection.Input, false, ((byte)(0)), ((byte)(0)), "Id", System.Data.DataRowVersion.Original, null)});
            // 
            // oleDbCommand5
            // 
            this.oleDbCommand5.CommandText = "INSERT INTO [QuestionSubtypes] ([Name]) VALUES (?)";
            ...
            // oleDbCommand6
            // 
            this.oleDbCommand6.CommandText = "SELECT     Id, Name\r\nFROM         QuestionSubtypes";
