...
namespace GmatClubTest.QuestionRenderer
{
	...
	{
		private MathML.Rendering.MathMLControl mathMLcontrol = new MathMLControl();
		private MathML.MathMLDocument mathMLDocument = new MathML.MathMLDocument(); 
		...
		public ImageSet Render(QuestionAnswerSet.QuestionsRow question)
		{
			ImageSet s = new ImageSet();
			 ...
			 InitBuffer(question.SubtypeId != (int)Question.Subtype.ReadingComprehensionQuestionToPassage ? largeBuffer : smallBuffer);
			...
			{
				InitBuffer(question.SubtypeId != (int)Question.Subtype.ReadingComprehensionQuestionToPassage ? largeBufferA : smallBufferA);
				RenderText(row.Text);
		...
		private void InitBuffer(Bitmap buffer)
		{
			...
			for(;;)
			{
				if (buffer == largeBuffer) {activeGraphics = largeGraphics; break;}
				if (buffer == largeBufferA) {activeGraphics = largeGraphicsA; break;}
				if (buffer == smallBuffer) {activeGraphics = smallGraphics; break;}
				if (buffer == smallBufferA) {activeGraphics = smallGraphicsA; break;}
			}
