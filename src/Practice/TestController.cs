...
namespace GmatClubTest.Practice
{
    ...
    public class TestController
    {
        ...
        public void TransitionOnSelectQuestion(int selectedSetNamberOfComboBox, int questionNamberOfList)
        {
            ...
            {
                MessageBox.Show("Random access to quest is impossible on this test.", MainForm.APP_CAPTION,
            ...
            navigator.SetActiveQuestion(
                navigator.QuestionsAnswers[selectedSetNamberOfComboBox].Questions[questionNamberOfList].Id);
            review.Close();
            ShowQuestion();
        }
        ...
        {
            if (navigator.GetQuestionStatus(question.Id).status == Question.Status.StatusType.ANSWER_IS_CORRECT ||
                navigator.GetQuestionStatus(question.Id).status == Question.Status.StatusType.ANSWER_IS_INCORRECT)
            {
                return true;
            }
            else
        ...
        {
            ResultDetailsForm resultDetailsForm =
                new ResultDetailsForm(navigator.ResultResultDetailSet.Results[0].Id, manager);
        ...
        private void CreateAnswerRadioButtons()
        {
            ...
            for (i = 0; i < imageSet.Answers.Length; i++)
            {
                ...
                {
                    controls =
                        InitAnswerRadioButton(x,
                                              answerPictureControls[i - 1].Location.Y +
                                              answerPictureControls[i - 1].Height + 5, i, imageSet.Answers[i]);
                }
                else
                {
        ...
        public void AnswerCheck_Click()
        {
            if (answerIndex == -1)
            {
                MessageBox.Show("Your answer is incorrect.", MainForm.APP_CAPTION, MessageBoxButtons.OK,
                                MessageBoxIcon.Warning);
                return;
            }
            if (answersRow[answerIndex].IsCorrect)
            {
                MessageBox.Show("Your answer is correct.", MainForm.APP_CAPTION, MessageBoxButtons.OK,
        ...
        public Control[] InitAnswerRadioButton(int locationX, int locationY, int idAnswer, Image answerImage)
        {
            ...
            answerPictureBox.Size = new Size(15, 15);
            answerPictureBox.SizeMode = PictureBoxSizeMode.AutoSize;
            ...
            answerRadioButton.Size = new Size(15, 15);
        ...
        public void ChangeStatusBar()
        {
            ...
            {
                statusBar.Text =
                    String.Format("{0:00}:{1:00}:{2:00}", navigator.RemainedTime.Hours, navigator.RemainedTime.Minutes,
                                  navigator.RemainedTime.Seconds);
            } //else
        ...
        {
            DialogResult result =
                MessageBox.Show("You will not be able to continue the test. Do you really want to quit?",
                                MainForm.APP_CAPTION, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
