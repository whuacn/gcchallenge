...
namespace GmatClubTest.ManualLicenseIssuer
{
	...
	public class MainForm : Form
	{
		...
		private System.Windows.Forms.SaveFileDialog saveKeyPairDialog;
		private System.Windows.Forms.OpenFileDialog openKeyDialog;
		private System.Windows.Forms.SaveFileDialog exportPublicKeyDialog;
		private System.Windows.Forms.OpenFileDialog openLicenseDialog;
		private System.Windows.Forms.SaveFileDialog saveLicenseDialog;
		...
		public MainForm()
		{
			...
			keyFileNameBox.Text = (String)registryKey.GetValue("KeyFileName", Application.StartupPath + "\\DefaultGmatClubKeyPair.gck");
		...
		private void InitializeComponent()
		{
			...
			this.licenseTypeCombo.SelectedIndexChanged += new System.EventHandler(this.licenseTypeCombo_SelectedIndexChanged);
			// 
			// label4
			// 
		...
		{
			if (isPrivateKeyOk)
				if (DialogResult.Yes != MessageBox.Show("You have a valid pair of keys. If you generate a new pair, you have to rebuild 'GmatClubTest - Practice' with inserted hash value of the new public key. Also you need to export the key and supply it to the program. Otherwise the program will not recognize newly issued licenses.\r\n\r\nDo you want to generate the new pair?", APP_CAPTION, MessageBoxButtons.YesNo, MessageBoxIcon.Warning))
					return;

			string s = License.Manager.GenerateKey();
		...
		private void exportButton_Click(object sender, System.EventArgs e)
		{
			if (exportPublicKeyDialog.ShowDialog(this) == DialogResult.OK)
			{
				...
				sw.Write(License.Manager.ExportPublicKey(dsa));
				sw.Close();

				sw = new StreamWriter(exportPublicKeyDialog.FileName + ".hash");
				byte[] hash = License.Manager.ComputeHash(exportPublicKeyDialog.FileName);
				sw.Write(Convert.ToBase64String(hash));
				sw.Close();
			}
		}

		private void browseKeyButton_Click(object sender, System.EventArgs e)
		{
		...
		private void licenseTypeCombo_SelectedIndexChanged(object sender, System.EventArgs e)
		{
		...
		private void issueButton_Click(object sender, System.EventArgs e)
		{
			...
			{
				XmlDocument doc = License.Manager.IssueLicense(new License.XmlLicense(userNameBox.Text, licenseTypeCombo.SelectedItem.ToString(), expiredDatePicker.Value, serialNumberBox.Text, issuedDatePicker.Value), dsa);
		...
		private void browselicenseButton_Click(object sender, System.EventArgs e)
		{
		...
		private void checkButton_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				License.XmlLicense l = License.Manager.CheckLicense(licenseFileNameBox.Text, keyFileNameBox.Text);
